        ...
        try {
            if (rs.next()) {
                for (i = 0; i < newrules.getRules().size(); i++) {
                    rule now = newrules.getRules().get(0);
                    String checkRule = "SELECT * FROM rule INNER JOIN service ON service.serviceid=rule.serviceid WHERE serviceurl = '?'";
                    ResultSet check = db.executeQuery(checkRule, newrules.getUrl());
